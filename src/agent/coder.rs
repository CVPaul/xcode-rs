use crate::agent::{Agent, AgentResult, truncate_messages};
use crate::config::AgentConfig;
use crate::llm::{Message, LlmProvider, ToolDefinition};
...
impl CoderAgent {
    pub fn new(config: AgentConfig) -> Self { CoderAgent { config } }
}
    ...
    async fn run(
        ...
        loop {
            ...
            if response.tool_calls.is_none() || response.tool_calls.as_ref().map(|t| t.is_empty()).unwrap_or(true) {
                let final_msg = response.content.unwrap_or_else(|| "Task completed.".to_string());
                println!("{}", final_msg);
            ...
            let to_execute = tool_calls.into_iter().take(self.config.max_tool_calls_per_response as usize);
            for tool_call in to_execute {
                ...
                let result = if let Some(tool) = tools.get(&tool_call.function.name) {
                    match tool.execute(args, ctx).await {
                        Ok(r) => r,
                        Err(e) => crate::tools::ToolResult { output: format!("Tool execution error: {}", e), is_error: true },
                    }
                } else {
...
mod tests {
    use super::*;
    use crate::config::AgentConfig;
    use crate::llm::{LlmResponse, Message, ToolCall, FunctionCall, ToolDefinition, Role};
    ...
    impl MockLlm {
        fn new(responses: Vec<LlmResponse>) -> Self {
            MockLlm { responses: Arc::new(Mutex::new(responses)) }
        }
    }
    #[async_trait]
    impl LlmProvider for MockLlm {
        async fn chat_completion(&self, _msgs: &[Message], _tools: &[ToolDefinition]) -> Result<LlmResponse> {
            let mut r = self.responses.lock().unwrap();
            if r.is_empty() {
                return Ok(LlmResponse { content: Some("done".to_string()), tool_calls: None });
            }
            Ok(r.remove(0))
        }
    }
    ...
    impl Tool for EchoTool {
        ...
        fn parameters_schema(&self) -> serde_json::Value { serde_json::json!({"type":"object","properties":{"text":{"type":"string"}}}) }
        async fn execute(&self, args: serde_json::Value, _ctx: &ToolContext) -> Result<ToolResult> {
            Ok(ToolResult { output: args["text"].as_str().unwrap_or("").to_string(), is_error: false })
        }
    }

    fn make_tool_call(id: &str, name: &str, args: &str) -> ToolCall {
        ToolCall { id: id.to_string(), call_type: "function".to_string(), function: FunctionCall { name: name.to_string(), arguments: args.to_string() } }
    }

    fn test_ctx() -> ToolContext {
        ToolContext { working_dir: std::path::PathBuf::from("/tmp"), sandbox_enabled: false }
    }

    fn test_config() -> AgentConfig {
        AgentConfig { max_iterations: 5, max_tool_calls_per_response: 3 }
    }

    #[tokio::test]
    async fn test_coder_simple_task() {
        let llm = MockLlm::new(vec![
            LlmResponse { content: None, tool_calls: Some(vec![make_tool_call("call1", "echo", r#"{"text":"hello"}"#)]) },
            LlmResponse { content: Some("Task complete!".to_string()), tool_calls: None },
        ]);
        let mut registry = ToolRegistry::new();
        registry.register(Box::new(EchoTool));
        let agent = CoderAgent::new(test_config());
        let mut messages = vec![Message::system(agent.system_prompt()), Message::user("say hello")];
        let result = agent.run(&mut messages, &registry, &llm, &test_ctx()).await.unwrap();
    ...
    async fn test_coder_max_iterations() {
        let responses: Vec<LlmResponse> = (0..20).map(|_| LlmResponse {
            content: None,
        ...
        let mut registry = ToolRegistry::new();
        registry.register(Box::new(EchoTool));
        let config = AgentConfig { max_iterations: 3, max_tool_calls_per_response: 1 };
        let agent = CoderAgent::new(config);
        let mut messages = vec![Message::system(agent.system_prompt()), Message::user("loop forever")];
        let result = agent.run(&mut messages, &registry, &llm, &test_ctx()).await.unwrap();
    ...
    async fn test_coder_max_tool_calls_per_response() {
        let tool_calls: Vec<ToolCall> = (0..5).map(|i| make_tool_call(&format!("call{}", i), "echo", r#"{"text":"x"}"#)).collect();
        let llm = MockLlm::new(vec![
            LlmResponse { content: None, tool_calls: Some(tool_calls) },
            LlmResponse { content: Some("done".to_string()), tool_calls: None },
        ]);
        let mut registry = ToolRegistry::new();
        registry.register(Box::new(EchoTool));
        let config = AgentConfig { max_iterations: 5, max_tool_calls_per_response: 2 };
        let agent = CoderAgent::new(config);
        let mut messages = vec![Message::system(agent.system_prompt()), Message::user("many tools")];
        let result = agent.run(&mut messages, &registry, &llm, &test_ctx()).await.unwrap();
    ...
    async fn test_coder_tool_error_recovery() {
        ...
        impl Tool for ErrorTool {
            ...
            async fn execute(&self, _: serde_json::Value, _: &ToolContext) -> Result<ToolResult> {
                Ok(ToolResult { output: "something went wrong".to_string(), is_error: true })
            }
        }
        let llm = MockLlm::new(vec![
            LlmResponse { content: None, tool_calls: Some(vec![make_tool_call("e1", "fail", "{}")]) },
            LlmResponse { content: Some("recovered".to_string()), tool_calls: None },
        ]);
        let mut registry = ToolRegistry::new();
        registry.register(Box::new(ErrorTool));
        let agent = CoderAgent::new(test_config());
        let mut messages = vec![Message::system(agent.system_prompt()), Message::user("test error")];
        let result = agent.run(&mut messages, &registry, &llm, &test_ctx()).await.unwrap();
        ...
        assert!(tool_result_msg.unwrap().content.as_ref().unwrap().contains("something went wrong"));
    }
    ...
    async fn test_context_truncation() {
        ...
        assert!(messages.len() < original_len, "Expected truncation but got {} msgs", messages.len());
